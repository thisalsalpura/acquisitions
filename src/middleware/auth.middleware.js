...
export const authenticateToken = (req, res, next) => {
    try {
        ...
        return res.status(500).json({ error: 'Internal server error', message: 'Error during authentication' });
    }
}
...
export const requireRole = allowedRoles => {
    return (req, res, next) => {
        try {
            ...
            return res.status(500).json({ error: 'Internal server error', message: 'Error during role verification' });
        }
    }
}